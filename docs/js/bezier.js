...
// math-inlining.
var abs$1 = Math.abs, cos$1 = Math.cos, sin$1 = Math.sin, acos$1 = Math.acos, atan2 = Math.atan2, sqrt$1 = Math.sqrt, pow = Math.pow;
// cube root function yielding real roots
function crt(v) {
    return v < 0 ? -pow(-v, 1 / 3) : pow(v, 1 / 3);
}
// trig constants
var pi$1 = Math.PI, tau = 2 * pi$1, quart = pi$1 / 2, 
...
var utils = {
    ...
    compute: function (t, points, _3d) {
        if (_3d === void 0) { _3d = false; }
        // shortcuts
        if (t === 0) {
            return __assign(__assign({}, points[0]), { t: 0 });
        }
        var order = points.length - 1;
        if (t === 1) {
            return __assign(__assign({}, points[order]), { t: 1 });
        }
        var mt = 1 - t;
        var p = points;
        // constant?
        if (order === 0) {
            return __assign(__assign({}, points[0]), { t: t });
        }
        // linear?
        if (order === 1) {
            var ret = {
            ...
            if (_3d) {
                return __assign(__assign({}, ret), { z: mt * p[0].z + t * p[1].z });
            }
            return ret;
        }
        // quadratic/cubic curve?
        if (order < 4) {
            var mt2 = mt * mt, t2 = t * t, a = void 0, b = void 0, c = void 0, d = 0;
            if (order === 2) {
                p = [p[0], p[1], p[2], ZERO];
                a = mt2;
                b = mt * t * 2;
            ...
            if (_3d) {
                return __assign(__assign({}, ret), { z: a * p[0].z + b * p[1].z + c * p[2].z + d * p[3].z });
            }
            return ret;
        }
    ...
    computeWithRatios: function (t, points, ratios, _3d) {
        var mt = 1 - t, r = ratios, p = points;
        var f1 = r[0], f2 = r[1], f3 = r[2], f4 = r[3], d;
        // spec for linear
        f1 *= mt;
        f2 *= t;
        if (p.length === 2) {
            ...
            return {
                ...
                z: !_3d ? undefined : (f1 * p[0].z + f2 * p[1].z) / d,
                t: t,
            };
        }
        ...
        if (p.length === 3) {
            ...
            return {
                ...
                z: !_3d ? undefined : (f1 * p[0].z + f2 * p[1].z + f3 * p[2].z) / d,
                t: t,
            };
        }
        // upgrade to cubic
    ...
    approximately: function (a, b, precision) {
        ...
        return abs$1(a - b) <= precision;
    },
    length: function (derivativeFn) {
        var z = 0.5, len = utils.Tvalues.length;
        var sum = 0;
        for (var i = 0, t = void 0; i < len; i++) {
    ...
    lerp: function (r, v1, v2) {
        ...
        if (!!v1.z && !!v2.z) {
            return __assign(__assign({}, ret), { z: v1.z + r * (v2.z - v1.z) });
        }
        return ret;
    },
    ...
    makeshape: function (forward, back, curveIntersectionThreshold) {
        ...
        return __assign(__assign({}, shape), { intersections: function (s2) {
    ...
    roots: function (points, line) {
        ...
        if (order === 2) {
            var a_1 = aligned[0].y, b_1 = aligned[1].y, c_1 = aligned[2].y, d_1 = a_1 - 2 * b_1 + c_1;
            if (d_1 !== 0) {
                var m1 = -sqrt$1(b_1 * b_1 - a_1 * c_1), m2 = -a_1 + b_1, v1_1 = -(m1 + m2) / d_1, v2 = -(-m1 + m2) / d_1;
                return [v1_1, v2].filter(reduce);
            }
            else if (b_1 !== c_1 && d_1 === 0) {
                return [(2 * b_1 - c_1) / (2 * b_1 - 2 * c_1)].filter(reduce);
            }
            return [];
        }
        // see http://www.trans4mind.com/personal_development/mathematics/polynomials/cubicAlgebra.htm
        var pa = aligned[0].y, pb = aligned[1].y, pc = aligned[2].y, pd = aligned[3].y;
        ...
        if (utils.approximately(d, 0)) {
            ...
            // quadratic solution:
            var q_1 = sqrt$1(b * b - 4 * a * c), a2 = 2 * a;
            return [(q_1 - b) / a2, (-b - q_1) / a2].filter(reduce);
        }
        ...
        var u1, v1, x1, x2, x3;
        if (discriminant < 0) {
            var mp3 = -p / 3, mp33 = mp3 * mp3 * mp3, r = sqrt$1(mp33), t = -q / (2 * r), cosphi = t < -1 ? -1 : t > 1 ? 1 : t, phi = acos$1(cosphi), crtr = crt(r), t1 = 2 * crtr;
    ...
    droots: function (p) {
        ...
        if (p.length === 3) {
            ...
            if (d !== 0) {
                var m1 = -sqrt$1(b * b - a * c), m2 = -a + b, v1 = -(m1 + m2) / d, v2 = -(-m1 + m2) / d;
                return [v1, v2];
            }
            else if (b !== c && d === 0) {
                return [(2 * b - c) / (2 * (b - c))];
    ...
    curvature: function (t, d1, d2, _3d, kOnly) {
        ...
        if (_3d) {
            num = sqrt$1(pow(d.y * dd.z - dd.y * d.z, 2) +
    ...
    inflections: function (points) {
        ...
        var trm = v2 * v2 - 4 * v1 * v3, sq = Math.sqrt(trm), d2 = 2 * v1;
    ...
    bboxoverlap: function (b1, b2) {
        var dims = ["x", "y"], len = dims.length;
        for (var i = 0, dim = void 0, l = void 0, t = void 0, d = void 0; i < len; i++) {
            dim = dims[i];
            l = b1[dim].mid;
            t = b2[dim].mid;
    ...
    pairiteration: function (c1, c2, curveIntersectionThreshold) {
        var c1b = c1.bbox(), c2b = c2.bbox(), // TODO: fixme
        r = 100000, threshold = curveIntersectionThreshold || 0.5;
        ...
        var cc1 = c1.splitSingle(0.5), cc2 = c2.splitSingle(0.5), pairs = [
    ...
    getccenter: function (p1, p2, p3) {
        var dx1 = p2.x - p1.x, dy1 = p2.y - p1.y, dx2 = p3.x - p2.x, dy2 = p3.y - p2.y, dx1p = dx1 * cos$1(quart) - dy1 * sin$1(quart), dy1p = dx1 * sin$1(quart) + dy1 * cos$1(quart), dx2p = dx2 * cos$1(quart) - dy2 * sin$1(quart), dy2p = dx2 * sin$1(quart) + dy2 * cos$1(quart), 
        // chord midpoints
        ...
        var s = atan2(p1.y - arc.y, p1.x - arc.x), m = atan2(p2.y - arc.y, p2.x - arc.x), e = atan2(p3.y - arc.y, p3.x - arc.x), _;
        ...
        // assign and done.
        return __assign(__assign({}, arc), { s: s,
            e: e,
            r: r });
    },
...
**/
// math-inlining.
var abs = Math.abs, min = Math.min, max = Math.max, cos = Math.cos, sin = Math.sin, acos = Math.acos, sqrt = Math.sqrt;
var pi = Math.PI;
/**
...
var Bezier = /** @class */ (function () {
    function Bezier(coords) {
        ...
        var aligned = utils.align(points, { p1: points[0], p2: points[order] });
        this._linear = !aligned.some(function (p) { return abs(p.y) > 0.0001; });
        this._lut = [];
        this._t1 = 0;
        this._t2 = 1;
    ...
    Object.defineProperty(Bezier, "PolyBezier", {
        get: function () {
            return PolyBezier;
        },
        enumerable: false,
    ...
    Bezier.prototype.setRatios = function (ratios) {
    ...
    Bezier.prototype.verify = function () {
    ...
    Bezier.prototype.coordDigest = function () {
        return this.points
            .map(function (c, pos) {
    ...
    Bezier.getABC = function (order, S, B, E, t) {
        ...
        return { A: A, B: B, C: C, S: S, E: E };
    };
    Bezier.prototype.getABC = function (t, B) {
        B = B || this.get(t);
        var S = this.points[0];
    ...
    Bezier.prototype.getLUT = function (steps) {
        ...
        steps--;
        for (var i = 0, p = void 0, t = void 0; i < steps; i++) {
            t = i / (steps - 1);
            p = this.compute(t);
            p.t = t;
            this._lut.push(p);
    ...
    Bezier.prototype.on = function (point, error) {
        error = error || 5;
        var lut = this.getLUT(), hits = [];
        var t = 0;
        for (var i = 0, c = void 0; i < lut.length; i++) {
            c = lut[i];
    ...
    Bezier.prototype.project = function (point) {
        // step 1: coarse check
        var LUT = this.getLUT(), l = LUT.length - 1, closest = utils.closest(LUT, point), mpos = closest.mpos, t1 = (mpos - 1) / l, t2 = (mpos + 1) / l, step = 0.1 / l;
        // step 2: fine check
        var mdist = closest.mdist, t = t1, ft = t, p;
        mdist += 1;
        for (var d = void 0; t < t2 + step; t += step) {
            p = this.compute(t);
            d = utils.dist(point, p);
            if (d < mdist) {
                mdist = d;
    ...
    Bezier.prototype.raise = function () {
        var p = this.points, np = [p[0]], k = p.length;
        for (var i = 1, pi_1, pim = void 0; i < k; i++) {
            pi_1 = p[i];
            pim = p[i - 1];
            np[i] = {
                x: ((k - i) / k) * pi_1.x + (i / k) * pim.x,
                y: ((k - i) / k) * pi_1.y + (i / k) * pim.y,
            };
        }
        np[k] = p[k - 1];
    ...
    Bezier.prototype.dderivative = function (t) {
        return utils.compute(t, this.dpoints[1], this._3d);
    };
    Bezier.prototype.align = function () {
        var p = this.points;
        return new Bezier(utils.align(p, { p1: p[0], p2: p[p.length - 1] }));
    };
    Bezier.prototype.curvature = function (t) {
    ...
    Bezier.prototype.hull = function (t) {
        ...
        while (p.length > 1) {
            _p = [];
            for (var i = 0, pt = void 0, l = p.length - 1; i < l; i++) {
    ...
    Bezier.prototype.splitSingle = function (t) {
        var q = this.hull(t);
        var left = this.order === 2
            ? new Bezier([q[0], q[3], q[5]])
            : new Bezier([q[0], q[4], q[7], q[9]]);
        var right = this.order === 2
    ...
    Bezier.prototype.splitDouble = function (t1, t2) {
        var result = this.splitSingle(t1);
        utils.map(t2, t1, 1, 0, 1);
    ...
    */
    ...
    Bezier.prototype.offsetD = function (t, d) {
        var c = this.get(t), n = this.normal(t);
        var ret = {
            c: c,
            n: n,
            x: c.x + n.x * d,
        ...
        if (this._3d) {
            return __assign(__assign({}, ret), { z: c.z + n.z * d });
        }
        return ret;
    };
    Bezier.prototype.offset = function (t) {
        if (this._linear) {
            var nv_1 = this.normal(0), coords = this.points.map(function (p) {
                var ret = {
                    x: p.x + t * nv_1.x,
                    y: p.y + t * nv_1.y,
                };
                if (p.z && nv_1.z) {
                    return __assign(__assign({}, ret), { z: p.z + t * nv_1.z });
                }
                return ret;
            });
    ...
    Bezier.prototype.scale = function (d) {
        ...
        var v = [this.offsetD(0, 10), this.offsetD(1, 10)];
        var points = this.points;
        var np = [];
    ...
    Bezier.prototype.outline = function (d1, d2, d3, d4) {
        ...
        var reduced = this.reduce(), len = reduced.length, fcurves = [];
        var bcurves = [], p, alen = 0, tlen = this.length();
        ...
        var fs = fcurves[0].points[0], fe = fcurves[len - 1].points[fcurves[len - 1].points.length - 1], bs = bcurves[len - 1].points[bcurves[len - 1].points.length - 1], be = bcurves[0].points[0], ls = utils.makeline(bs, fs), le = utils.makeline(fe, be), segments = [ls].concat(fcurves).concat([le]).concat(bcurves);
    ...
    Bezier.prototype.intersects = function (curve, curveIntersectionThreshold) {
        if (!curve)
    ...
    Bezier.prototype.lineIntersects = function (line) {
        var _this = this;
        var mx = min(line.p1.x, line.p2.x), my = min(line.p1.y, line.p2.y), MX = max(line.p1.x, line.p2.x), MY = max(line.p1.y, line.p2.y);
    ...
    Bezier.prototype.selfintersects = function (curveIntersectionThreshold) {
        ...
        var reduced = this.reduce(), len = reduced.length - 2, results = [];
        for (var i = 0, result = void 0, left = void 0, right = void 0; i < len; i++) {
    ...
    Bezier.prototype._iterate = function (errorThreshold, circles) {
        ...
        do {
            ...
            t_e = 1;
            // points:
            var np1 = this.get(t_s), np2 = void 0, np3 = void 0, arc = void 0, prev_arc = void 0;
            // booleans:
            var curr_good = false, prev_good = false, done = void 0;
            // numbers:
